  ...
  class PatchedEditFieldMappingRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MergeHRISClient::PatchedEditFieldMappingRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MergeHRISClient::PatchedEditFieldMappingRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      return false if !@remote_method.nil? && @remote_method.to_s.length < 1
      return false if !@remote_url_path.nil? && @remote_url_path.to_s.length < 1
      true
    end
    ...
    def hash
      [remote_field_traversal_path, remote_method, remote_url_path].hash
    end
