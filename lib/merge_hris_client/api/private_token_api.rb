  ...
  class PrivateTokenApi
    ...
    def private_token_retrieve_with_http_info(public_token, opts = {})
      ...
      if @api_client.config.client_side_validation && public_token.nil?
        fail ArgumentError, "Missing the required parameter 'public_token' when calling PrivateTokenApi.private_token_retrieve"
      end
      # resource path
      local_var_path = '/private-token/{public_token}'.sub('{' + 'public_token' + '}', CGI.escape(public_token.to_s))
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: PrivateTokenApi#private_token_retrieve\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
