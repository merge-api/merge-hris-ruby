=begin
#Merge HRIS API

#The unified API for building rich integrations with multiple HR Information System platforms.

The version of the OpenAPI document: 1.0
Contact: hello@merge.dev
Generated by: https://openapi-generator.tech
OpenAPI Generator version: 4.3.1

=end

require 'spec_helper'
require 'json'

# Unit tests for OpenapiClient::DeductionsApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'DeductionsApi' do
  before do
    # run before each test
    @api_instance = OpenapiClient::DeductionsApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of DeductionsApi' do
    it 'should create an instance of DeductionsApi' do
      expect(@api_instance).to be_instance_of(OpenapiClient::DeductionsApi)
    end
  end

  # unit tests for deductions_create
  # Creates a &#x60;Deduction&#x60; object with the given values.
  # @param [Hash] opts the optional parameters
  # @option opts [Boolean] :run_async Whether or not third-party updates should be run asynchronously.
  # @option opts [Deduction] :deduction 
  # @return [Deduction]
  describe 'deductions_create test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for deductions_destroy
  # Deletes a &#x60;Deduction&#x60; object with the given &#x60;id&#x60;.
  # @param id 
  # @param [Hash] opts the optional parameters
  # @option opts [Boolean] :run_async Whether or not third-party updates should be run asynchronously.
  # @return [AsyncTaskExecution]
  describe 'deductions_destroy test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for deductions_list
  # Returns a list of &#x60;Deduction&#x60; objects.
  # @param [Hash] opts the optional parameters
  # @option opts [Integer] :cursor The pagination cursor value.
  # @option opts [String] :linked_account_id If provided, will only return objects associated with the given &#x60;linked_account_id&#x60;.
  # @option opts [String] :remote_id The API provider&#39;s ID for the given object.
  # @return [PaginatedDeductionList]
  describe 'deductions_list test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for deductions_partial_update
  # Updates a &#x60;Deduction&#x60; object with the given &#x60;id&#x60;.
  # @param id 
  # @param [Hash] opts the optional parameters
  # @option opts [Boolean] :run_async Whether or not third-party updates should be run asynchronously.
  # @option opts [PatchedDeduction] :patched_deduction 
  # @return [Deduction]
  describe 'deductions_partial_update test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for deductions_retrieve
  # Returns a &#x60;Deduction&#x60; object with the given &#x60;id&#x60;.
  # @param id 
  # @param [Hash] opts the optional parameters
  # @return [Deduction]
  describe 'deductions_retrieve test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

  # unit tests for deductions_update
  # @param id 
  # @param [Hash] opts the optional parameters
  # @option opts [Deduction] :deduction 
  # @return [Deduction]
  describe 'deductions_update test' do
    it 'should work' do
      # assertion here. ref: https://www.relishapp.com/rspec/rspec-expectations/docs/built-in-matchers
    end
  end

end
